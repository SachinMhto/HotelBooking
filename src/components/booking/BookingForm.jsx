...
const BookingForm = () => {
  ...
  const [displayBookingSummary, setDisplayBookingSummary] = useState(false);
  ...
  const handleFormSubmit = async () => {
    try {
      ...
      // Send confirmationCode to the backend
      const response = await fetch("http://localhost:8080/code/confirmation", {
        method: "POST",
        headers: {
          ...
          <div className="col-md-4">
            {isSubmitted && (
              <BookingSummary
                ...
                display={displayBookingSummary} // Pass the state as a prop
              />
            )}
          </div>
        </div>
